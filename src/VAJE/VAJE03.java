...
public class VAJE03 {
    ...
    static void izpisi16bit(short kodaZnaka){
        for (int i = 3; i >= 0; i--) {
            for (int j = 3; j >= 0; j--) {
                ...
                System.out.print((kodaZnaka & maska) == maska ? Znaki.belaPika : Znaki.crnaPika);
            }
            System.out.println();
        }
    ...
    static void izpisi16bit(short[] nizZnakov){
        for (int i = 3; i >= 0; i--) {
            for (short znak : nizZnakov) {
                for (int j = 3; j >= 0; j--) {
                    short maska = (short) (1 << 4 * i + j);
                    System.out.print((znak & maska) == maska ? Znaki.belaPika : Znaki.crnaPika);
                }
    ...
    static int indexZnaka(char znak){
        for (int i = 0; i < Znaki.abeceda.length; i++){
            if (znak == Znaki.abeceda[i]){
                return i;
            }
        }
        return Znaki.abeceda.length - 1;
    }
    static void izpisi16bit(String niz) {
        niz = niz.toUpperCase();
        short[] Znak = new short[niz.length()];
        for (int i = 0; i < niz.length(); i++){
            Znak[i] = Znaki.kodeZnakov16bit[indexZnaka(niz.charAt(i))];
        }
        izpisi16bit(Znak);
    }
    ...
    static void izpisi64bit(String niz){
        niz = niz.toUpperCase();
        long[] Znak = new long[niz.length()];
        for (int i = 0; i < niz.length(); i++){
            Znak[i] = Znaki.kodeZnakov64bit[indexZnaka(niz.charAt(i))];
        }
        izpisi64bit(Znak);
    }
    public static void main(String[] args){
        izpisi16bit((short) 0b1111100011101000);
        izpisi16bit(new short[] {(short)0b1111100011101000, (short)0b1111100111111010, (short)0b1111010001001111});
        izpisi16bit("Dale a tu cuerpo alegria macarena");
        izpisi64bit(0b1111111001000001010000010100000101111110010000000100000011100000L);
